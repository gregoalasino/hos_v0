...
export default function AdminClasesPage() {
  ...
  const [clases, setClases] = useState<YogaClass[]>(
    [...MOCK_CLASSES].sort((a, b) => a.startsAt.getTime() - b.startsAt.getTime())
  );
  const [modalOpen, setModalOpen] = useState(false);
  const [editingClass, setEditingClass] = useState<YogaClass | undefined>();
  const [deletingId, setDeletingId] = useState<string | null>(null);

  function handleSave(data: YogaClass) {
    setClases((prev) =>
      prev.some((c) => c.id === data.id)
        ? prev.map((c) => (c.id === data.id ? data : c))
        : [...prev, data].sort((a, b) => a.startsAt.getTime() - b.startsAt.getTime())
    );
  }

  function handleToggleActive(id: string) {
    setClases((prev) => prev.map((c) => (c.id === id ? { ...c, isActive: !c.isActive } : c)));
  }
              ...
              {clases.map((clase) => (
                <tr key={clase.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3 hidden xl:table-cell">
                    <span className="font-mono text-xs text-slate-400 bg-gray-100 px-2 py-0.5 rounded">
                      {clase.slug}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-slate-500 hidden md:table-cell">{clase.instructor}</td>
                  <td className="px-4 py-3">
                    <p className="text-slate-700 capitalize">{format(clase.startsAt, lang === 'es' ? "d MMM yyyy" : "MMM d, yyyy", { locale: dateFnsLocale })}</p>
