...
import {
  ...
  CalendarDays, MapPin, User, DollarSign, Pencil, X,
} from 'lucide-react';
import { MOCK_CLASSES, MOCK_BOOKINGS } from '@/lib/mock-data';
...
function classTopPx(startsAt: Date): number {
  const minutes = startsAt.getHours() * 60 + startsAt.getMinutes() - GRID_START_HOUR * 60;
  return Math.max(0, (minutes / 60) * HOUR_PX);
}
function classHeightPx(durationMinutes: number): number {
  return (durationMinutes / 60) * HOUR_PX;
}
...
export default function CalendarioPage() {
  ...
  const classMap = useMemo(() => {
    const map = new Map<number, YogaClass[]>();
    weekDays.forEach((_, i) => map.set(i, []));
    MOCK_CLASSES.filter(c => c.isActive).forEach(c => {
      const dayIdx = weekDays.findIndex(d => isSameDay(d, c.startsAt));
  ...
  const classBookings = useMemo(
    () => selectedClass ? MOCK_BOOKINGS.filter(b => b.classId === selectedClass.id) : [],
    [selectedClass]
  );

  const occupied = selectedClass
    ? selectedClass.capacity - selectedClass.spotsRemaining
    : 0;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-5 py-4 bg-white border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-0.5">
            <button
              ...
              className="p-1.5 rounded-md hover:bg-gray-100 text-slate-500 transition-colors"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button
              onClick={() => setWeekOffset(0)}
              className="px-3 py-1.5 text-xs font-medium rounded-md hover:bg-gray-100 text-slate-600 transition-colors"
            >
              {tr(lang, 'Hoy', 'Today')}
            </button>
            <button
        ...
        <Button size="sm" className="gap-1.5 text-xs text-white" style={{ backgroundColor: '#c4622d' }}>
          ...
          {selectedClass && (
            <div className="flex flex-col min-h-full">
              ...
              <div
                ...
                style={{ background: selectedClass.color + '18' }}
              >
                {/* Category chip + class name */}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] text-slate-500">
                      {tr(lang, 'Ocupación', 'Occupancy')}
                    </span>
                    <span className="text-[11px] font-semibold text-slate-700">
                      {occupied}/{selectedClass.capacity} {tr(lang, 'reservas', 'bookings')}
                    </span>
                  </div>
                  <div className="h-2 bg-white/60 rounded-full overflow-hidden border border-white/40">
                    <div
                      className="h-full rounded-full transition-all duration-500"
                      style={{ width: `${occupancyPct}%`, background: selectedClass.color }}
                    />
                  </div>
                  <p className="text-[10px] text-slate-400 mt-1.5">
                    {selectedClass.spotsRemaining} {tr(lang, 'lugares disponibles', 'spots remaining')}
                  </p>
                </div>
              </div>
              ...
              <div className="px-6 py-5 space-y-4 border-b border-gray-100">
                <InfoItem
                  ...
                  label={tr(lang, 'Instructor', 'Instructor')}
                  value={selectedClass.instructor}
                />
                <InfoItem
                  icon={<CalendarDays className="w-3.5 h-3.5" />}
                  label={tr(lang, 'Fecha y hora', 'Date & time')}
                  value={
                    lang === 'es'
                      ? format(selectedClass.startsAt, "EEEE d 'de' MMMM · HH:mm", { locale: es })
                      : format(selectedClass.startsAt, 'EEEE, MMMM d · h:mm a')
                  }
                />
                <InfoItem
                  icon={<Clock className="w-3.5 h-3.5" />}
                  label={tr(lang, 'Duración', 'Duration')}
                  value={`${selectedClass.durationMinutes} ${tr(lang, 'minutos', 'min')}`}
                />
                <InfoItem
                  icon={<MapPin className="w-3.5 h-3.5" />}
                  label={tr(lang, 'Ubicación', 'Location')}
                  value={selectedClass.location}
                />
                <InfoItem
                  icon={<DollarSign className="w-3.5 h-3.5" />}
                  label={tr(lang, 'Precio', 'Price')}
                  value={
                    selectedClass.priceUsd === 0
                      ? tr(lang, 'Gratis', 'Free')
                      : `$${selectedClass.priceUsd} USD`
                  }
                />
              </div>
              ...
              <div className="px-6 py-4 flex gap-2 border-b border-gray-100">
                ...
                <Button
                  size="sm"
                  className="flex-1 text-xs gap-1.5 h-9 text-white"
                  style={{ backgroundColor: '#4a7c59' }}
                >
                  <Plus className="w-3.5 h-3.5" />
                  {tr(lang, 'Reserva manual', 'Add booking')}
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                ...
                {classBookings.length === 0 ? (
                  <div className="flex items-center justify-center py-10 rounded-xl border-2 border-dashed border-gray-100">
                    <p className="text-sm text-slate-400 italic">
                      {tr(lang, 'Sin reservas todavía', 'No bookings yet')}
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {classBookings.map(b => {
                      ...
                      return (
                        <div
                          key={b.id}
                          className="flex items-center gap-3 p-3 rounded-xl bg-gray-50/80 hover:bg-gray-100 transition-colors"
                        >
                          {/* Avatar */}
                          <div
...
// ── ClassBlock ────────────────────────────────────────────────────────────────
function ClassBlock({ clase, onClick }: { clase: YogaClass; onClick: () => void }) {
  const top    = classTopPx(clase.startsAt);
  const height = classHeightPx(clase.durationMinutes);
  ...
  return (
    <button
      ...
      style={{ top, height: Math.max(height, 36), backgroundColor: clase.color + 'dd' }}
    >
      <p className="text-[10px] font-semibold text-white leading-tight truncate">{clase.name}</p>
      <p className="text-[9px] text-white/80 truncate">{format(clase.startsAt, 'HH:mm')}</p>
      {height >= 52 && (
        <p className="text-[9px] text-white/70 mt-0.5">{filled}/{clase.capacity}</p>
      )}
    </button>
  );
}
