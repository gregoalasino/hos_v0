...
import { MOCK_BOOKINGS, MOCK_CLASSES, MOCK_UPSELLS } from '@/lib/mock-data';
...
export default function AdminReservasPage() {
  ...
  const claseOptions = useMemo(
    () => [...new Set(MOCK_BOOKINGS.map(b => b.className))].sort(),
    []
  );
  ...
  }, [search, filtroEstado, filtroClase]);

  function openDetail(b: Booking) { setSelected(b); setDrawerOpen(true); }

  const selectedClase   = selected ? MOCK_CLASSES.find(c => c.id === selected.classId) : null;
  const selectedUpsells = selected ? MOCK_UPSELLS.filter(u => selected.upsells.includes(u.id)) : [];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {filtered.length} {tr(lang, 'reservas', 'bookings')}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <Select value={filtroEstado} onValueChange={setFiltroEstado}>
          ...
          <SelectContent>
            ...
            <SelectItem value="free">{tr(lang, 'Gratis', 'Free')}</SelectItem>
          ...
          <table className="w-full text-sm">
                ...
                {[
                  ...
                  <th
                    ...
                    className={`text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide ${
                      ...
                      h === tr(lang, 'Personas', 'Pax') ||
                      h === tr(lang, 'Total', 'Total')                        ? 'hidden lg:table-cell' : ''
                    }`}
                  >
                    {h}
                  </th>
            ...
            <tbody>
              {filtered.map(b => {
                ...
                const total  = clase
                  ? clase.priceUsd * b.persons + MOCK_UPSELLS.filter(u => b.upsells.includes(u.id)).reduce((s, u) => s + u.priceUsd, 0)
                  : 0;
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {clase && <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: clase.color }} />}
                    ...
                    <td className="px-4 py-3 text-slate-500 hidden md:table-cell">
                      {clase
                        ? (lang === 'es'
                            ? format(clase.startsAt, "d MMM · HH:mm", { locale: es })
                            : format(clase.startsAt, 'MMM d · h:mm a'))
                        : '—'}
                    </td>
                    <td className="px-4 py-3 text-slate-600 hidden lg:table-cell">{b.persons}</td>
                    <td className="px-4 py-3 hidden lg:table-cell">
                      {b.paymentStatus === 'free'
                        ? <span className="text-slate-400 text-xs">{tr(lang, 'gratis', 'free')}</span>
          ...
          {selected && (
            <div className="flex flex-col min-h-full">
              ...
              <div className="px-6 pt-10 pb-5 border-b border-gray-100 bg-gray-50/60">
                ...
                <Badge
                  variant="outline"
                  className={`text-xs ${getStatusMeta(selected.paymentStatus, lang).cls}`}
                >
              ...
              <div className="px-6 py-4 flex gap-2 border-b border-gray-100">
                {selected.paymentStatus === 'pending' && (
                  <Button size="sm" className="text-xs text-white flex-1 gap-1.5 h-9" style={{ backgroundColor: SAGE }}>
                    <Check className="w-3.5 h-3.5" />
                    {tr(lang, 'Confirmar', 'Confirm')}
                  </Button>
                )}
                <Button size="sm" variant="outline" className="text-xs flex-1 h-9">
                  {tr(lang, 'Enviar email', 'Send email')}
                </Button>
                ...
                <div className="space-y-3">
                  ...
                  {selected.referralCode && (
                    <DrawerRow
                      ...
                      label={tr(lang, 'Código de referido', 'Referral code')}
                  ...
                  <DrawerRow
                    ...
                    label={tr(lang, 'Reservado el', 'Booked on')}
                    value={dateStr(selected.createdAt)}
                  />
                </div>
              </div>
              ...
              {selectedClase && (
                <div className="px-6 py-5 border-b border-gray-100">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: selectedClase.color }} />
                    <span className="text-sm font-semibold text-slate-800">{selectedClase.name}</span>
                  </div>
                  <div className="space-y-3">
                    <DrawerRow icon={<User className="w-3.5 h-3.5" />}       label={tr(lang, 'Instructor', 'Instructor')} value={selectedClase.instructor} />
                    <DrawerRow icon={<CalendarDays className="w-3.5 h-3.5" />} label={tr(lang, 'Fecha', 'Date')} value={dateStr(selectedClase.startsAt)} />
                    <DrawerRow icon={<MapPin className="w-3.5 h-3.5" />}     label={tr(lang, 'Ubicación', 'Location')} value={selectedClase.location} />
                    <DrawerRow icon={<User className="w-3.5 h-3.5" />}       label={tr(lang, 'Personas', 'Persons')} value={String(selected.persons)} />
                  </div>
                </div>
              )}
    ...
    <div className="flex items-start gap-2.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p
          ...
          style={{ color: highlight ? '#c4a030' : '#1e293b', fontWeight: highlight ? 600 : 400 }}
        >
          {value}
        </p>
      </div>
